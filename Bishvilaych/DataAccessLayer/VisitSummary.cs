  ...
  public  class VisitSummary//created by hila elgrabli 
    {
        ...
        public DataSet getFollowUp(ListDictionary Params) //קבלת רשימת פרמטרים
        {
            ...
            string sp_name = "getFollowUp"; //שם הפרוצדורה
        ...
        public DataSet getReccomendations(ListDictionary Params) //קבלת רשימת פרמטרים
        {
            DBCtrl db_ctrl = new DBCtrl();
            if (!db_ctrl.isConnected())
            {
        ...
        public DataSet get_Updating(ListDictionary Params) //קבלת רשימת פרמטרים
        {
            ...
            string sp_name = "get_Updating"; //שם הפרוצדורה
            DataSet retDataDs = new DataSet(); //הכנת הטבלה אליה יכנסו הנתונים
            string retError;
        ...
        public int addOrUpdateReccomendations(ListDictionary Params) //קבלת רשימת פרמטרים
        {
            ...
            string sp_name = "addOrUpdateReccomendations"; //שם הפרוצדורה

            return db_ctrl.ExecuteNonQueryFunction(connStr, sp_name, ref Params); //החזרת תוצאה
        }

        public int UpdateReccomendations(ListDictionary Params) //קבלת רשימת פרמטרים
        {
            DBCtrl db_ctrl = new DBCtrl();
            if (!db_ctrl.isConnected())
            {
            ...
            string sp_name = "UpdateReccomendations"; //שם הפרוצדורה

            return db_ctrl.ExecuteNonQueryFunction(connStr, sp_name, ref Params); //החזרת תוצאה
        }
        public int UpdateSummary(ListDictionary Params) //קבלת רשימת פרמטרים
        {
            DBCtrl db_ctrl = new DBCtrl();
            if (!db_ctrl.isConnected())
            {
            ...
            string sp_name = "UpdateSummary"; //שם הפרוצדורה
